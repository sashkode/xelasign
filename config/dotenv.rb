...
if ENV['RAILS_ENV'] == 'production'
  if Process.uid.zero?
    begin
      ...
      if File.exist?(workdir) && File.stat(workdir).uid != 2000
        puts 'Changing the owner of the xelasign directory...' unless Dir.empty?(workdir)

        FileUtils.chown_R(2000, 2000, workdir)
      end
    rescue StandardError
      puts 'Unable to change xelasign directory owner'
    end
  end
  ...
  if !ENV['AWS_SECRET_MANAGER_ID'].to_s.empty?
    ...
    dotenv_path = "#{ENV.fetch('WORKDIR', '.')}/xelasign.env"
